...
import { Plus, ArrowUpRight, ArrowDownLeft, AlertTriangle, Copy, CheckCheck } from 'lucide-react';
...
const statusColor = s => ({ COMPLETED:'green', DELAYED:'amber', REVERSED:'blue', REPORTED:'red', CANCELLED:'gray' })[s] || 'gray';

/* ── one-click UUID copy ── */
function UUIDDisplay({ value }) {
  ...
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:'6px' }}>
      <span style={{ fontFamily:'monospace', fontSize:'11px', color:'var(--accent-blue)', wordBreak:'break-all' }}>{value}</span>
      <button onClick={copy} title={copied ? 'Copied!' : 'Copy'}
        style={{ background:'none', border:'none', cursor:'pointer', padding:'2px', display:'inline-flex',
          color: copied ? 'var(--accent-green)' : 'var(--text-muted)', flexShrink:0 }}>
...
export default function UserDashboard() {
  ...
  const [txForm,     setTxForm]     = useState({
    amount:'', transaction_type:'debit',
    ip_address:'127.0.0.1', device_id:'web-001',
    location:'', channel:'web', transaction_duration:'1.5',
  });
  ...
  const doCreate = async e => {
    ...
    try {
      ...
      show(
        txForm.transaction_type === 'debit'
          ? `✅ Debit of ₹${parseFloat(txForm.amount).toLocaleString('en-IN')} submitted successfully`
          : `✅ Credit of ₹${parseFloat(txForm.amount).toLocaleString('en-IN')} submitted successfully`,
        'success'
      );
      setTxModal(false);
      ...
      show(err.response?.data?.detail || 'Transaction failed — please try again', 'error');
  ...
  const doReport = async () => {
    ...
    try {
      ...
      show('OTP sent to your registered email', 'success');
      setReportModal(m => ({ ...m, step:'otp' }));
    } catch(err) { show(err.response?.data?.detail || 'Could not initiate report', 'error'); }
    finally { setSubmitting(false); }
  };
  ...
  const doVerifyReport = async () => {
    ...
    try {
      ...
      show('✅ Fraud report submitted — our team will review it', 'success');
      setReportModal({ open:false, txId:null, step:'report' }); setOtp(''); load();
    } catch(err) { show(err.response?.data?.detail || 'Invalid OTP — please try again', 'error'); }
    finally { setSubmitting(false); }
  };
  ...
  return (
    <div>
        ...
        <div>
          ...
          {user?.id && (
            <div style={{ marginTop:'8px', display:'flex', alignItems:'center', gap:'6px' }}>
              <span style={{ fontSize:'11px', color:'var(--text-muted)' }}>Your ID:</span>
              <UUIDDisplay value={String(user.id)}/>
            </div>
          )}
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:'16px', marginBottom:'24px' }}>
        <StatCard label="Account Balance"
          value={`₹${(user?.account_balance || 0).toLocaleString('en-IN', { minimumFractionDigits:2 })}`}
          icon="💰" color="blue"/>
        <StatCard label="Total Debited"
          value={`₹${totalDebited.toLocaleString('en-IN', { minimumFractionDigits:2 })}`}
          icon="📤" color="red" sub={`${debits.length} transactions`}/>
        <StatCard label="Total Credited"
          value={`₹${totalCredited.toLocaleString('en-IN', { minimumFractionDigits:2 })}`}
          icon="📥" color="green" sub={`${credits.length} transactions`}/>
        <StatCard label="Fraud Alerts"
          value={fraudCount} icon="🚨" color="red"
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px', marginBottom:'24px' }}>
              ...
              <ResponsiveContainer width="55%" height={180}>
                <PieChart>
                  <Pie data={statusPie} cx="50%" cy="50%" outerRadius={80} dataKey="value" paddingAngle={2}>
                    {statusPie.map((d, i) => (
                      <Cell key={i} fill={
                        d.name==='COMPLETED'?'#10b981':d.name==='DELAYED'?'#f59e0b':d.name==='REPORTED'?'#ef4444':d.name==='REVERSED'?'#3b82f6':'#64748b'
                      }/>
                    ))}
                  </Pie>
                  <Tooltip contentStyle={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:8 }}/>
                ...
                {statusPie.map((d, i) => (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'8px', fontSize:'12px' }}>
                    <div style={{ width:10, height:10, borderRadius:'50%', flexShrink:0,
                      background: d.name==='COMPLETED'?'#10b981':d.name==='DELAYED'?'#f59e0b':d.name==='REPORTED'?'#ef4444':'#3b82f6' }}/>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <PieChart>
              <Pie
                data={[{ name:'Debited', value:totalDebited }, { name:'Credited', value:totalCredited }]}
      ...
      <Card>
        ...
        <Table
          ...
          rows={txns.slice(0, 10).map(t => (
            <>
              ...
              <td style={{ padding:'10px 14px' }}>
                {t.is_fraud ? <Badge color="red">⚠ Fraud</Badge> : <Badge color="green">Clear</Badge>}
              </td>
              <td style={{ padding:'10px 14px' }}>
                {t.status === 'DELAYED' && (
                  <Btn color="amber" size="sm"
                    onClick={() => setReportModal({ open:true, txId:t.id || t.public_id, step:'report' })}>
          ...
          empty="No transactions yet — create one using the button above!"/>
      </Card>

      {/* ── New Transaction modal ── */}
      <Modal open={txModal} onClose={() => setTxModal(false)} title="New Transaction">
        <form onSubmit={doCreate}>
          <Select label="Transaction Type" value={txForm.transaction_type}
            onChange={e => setTxForm(f => ({ ...f, transaction_type:e.target.value }))}>
            <option value="debit">Debit (send / spend money)</option>
            <option value="credit">Credit (receive money)</option>
          </Select>
          <Input label="Amount (₹)" type="number" step="0.01" min="0.01" placeholder="500.00"
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
            <Input label="IP Address" placeholder="192.168.1.1"
              value={txForm.ip_address} onChange={e => setTxForm(f => ({ ...f, ip_address:e.target.value }))} required/>
            <Input label="Device ID" placeholder="device-001"
              value={txForm.device_id} onChange={e => setTxForm(f => ({ ...f, device_id:e.target.value }))} required/>
          </div>
          <Select label="Channel" value={txForm.channel}
            onChange={e => setTxForm(f => ({ ...f, channel:e.target.value }))}>
          ...
          <div style={{ background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.2)',
            borderRadius:8, padding:'10px 12px', marginBottom:'16px', fontSize:'12px', color:'var(--accent-amber)' }}>
      ...
      {/* ── Report modal ── */}
      <Modal open={reportModal.open}
        onClose={() => setReportModal({ open:false, txId:null, step:'report' })}
        title="Report Fraudulent Transaction">
        {reportModal.step === 'report' ? (
          <div>
            <p style={{ color:'var(--text-secondary)', fontSize:'14px', marginBottom:'20px' }}>
              An OTP will be sent to your registered email to confirm this fraud report.
            </p>
            <Btn loading={submitting} onClick={doReport} size="lg" color="amber"
          ...
          <div>
            ...
            <Input label="OTP" placeholder="123456" value={otp}
              onChange={e => setOtp(e.target.value)} maxLength={6}/>
            <Btn loading={submitting} onClick={doVerifyReport} size="lg" color="red"
              style={{ width:'100%', justifyContent:'center' }}>
